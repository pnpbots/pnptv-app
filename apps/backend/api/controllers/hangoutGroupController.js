  ...
  try {
    ...
    await query(
      `INSERT INTO chat_messages (room, user_id, username, first_name, content)
       VALUES ($1, NULL, 'cristina', 'Cristina', $2)`,
...
const promoteMember = async (req, res) => {
  ...
  try {
    ...
    // Set Matrix power level to 50 (moderator) — use Telegram ID, not DB ID
    const { rows: tgRows } = await query('SELECT telegram FROM users WHERE id = $1', [targetId]);
    const telegramId = tgRows[0]?.telegram;
    if (telegramId) {
      const serverName = process.env.MATRIX_SERVER_NAME || 'matrix.pnptv.app';
      const matrixUserId = `@pnptv_${telegramId}:${serverName}`;
...
const demoteMember = async (req, res) => {
  ...
  try {
    ...
    // Set Matrix power level back to 0 (regular member) — use Telegram ID, not DB ID
    const { rows: tgRowsDemote } = await query('SELECT telegram FROM users WHERE id = $1', [targetId]);
    const telegramIdDemote = tgRowsDemote[0]?.telegram;
    if (telegramIdDemote) {
      const serverName = process.env.MATRIX_SERVER_NAME || 'matrix.pnptv.app';
      const matrixUserId = `@pnptv_${telegramIdDemote}:${serverName}`;
...
async function getVideoChatStatus(req, res) {
  ...
  try {
    ...
    // Membership gate — only members (and admins) may retrieve the Telegram invite link
    const { rows: memberCheck } = await query(
...
async function unlinkTelegramGroup(req, res) {
  ...
  try {
    ...
    // Fetch telegram_chat_id before nulling it so we can clear the correct cache key
    const { rows: oldRows } = await query(
      'SELECT telegram_chat_id FROM hangout_groups WHERE id = $1',
      [groupId]
    );
