...
const sendHangoutMessage = async (req, res) => {
  ...
  try {
    ...
    // Membership check — also reject banned members
    ...
    (async () => {
      try {
        const membersResult = await query(
          'SELECT user_id FROM hangout_group_members WHERE group_id = $1 AND user_id != $2 AND (notification_mode IS NULL OR notification_mode != \'muted\')',
          [groupId, user.id]
        );
