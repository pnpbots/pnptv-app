...
// ==========================================
// PNPtv Hybrid Feature & Monitoring Endpoints
// ==========================================

const {
  healthCheck: healthCheckHandler, authStatus: monitoringAuthStatus, runAuthTests,
...
app.get('/api/features/videorama/url', ...featureAuthMiddleware, (req, res) => {
  try {
    const videoramaUrl = process.env.VIDEORAMA_WEB_URL;
    if (!videoramaUrl) {
...
app.get('/api/features/nearby/url', ...featureAuthMiddleware, (req, res) => {
  try {
    const nearbyUrl = process.env.NEARBY_WEB_URL;
    if (!nearbyUrl) {
...
const requireAdmin = (req, res, next) => {
  ...
  if (!ADMIN_USER_IDS.includes(telegramId)) {
    return res.status(403).json({ error: 'Admin access required' });
  }
  next();
};

app.get('/api/monitor/auth-status', telegramAuth, requireAdmin, monitoringAuthStatus);
