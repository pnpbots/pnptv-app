...
function initSocketIO(io) {
  ...
  io.on('connection', async (socket) => {
        ...
        (async () => {
          try {
            ...
            const [groupResult, membersResult] = await Promise.all([
              ...
              query('SELECT user_id FROM hangout_group_members WHERE group_id = $1 AND user_id != $2 AND (notification_mode IS NULL OR notification_mode != \'muted\')', [gid, user.id]),
            ]);
    ...
    // ── WebRTC signaling (tgcalls mesh) ──────────────────────────────────────
    // Pure relay: the server never inspects SDP or ICE content.
    // All payloads are validated for type and size before forwarding.

    socket.on('webrtc:join', ({ groupId, displayName } = {}) => {
      const gid = parseInt(groupId, 10)
      if (!Number.isFinite(gid)) return
      const room = `hangout:${gid}`
      // Only relay if socket is actually in the room (joined via hangout:join)
      if (!socket.rooms.has(room)) return
      socket.to(room).emit('webrtc:user-joined', {
        peerId: socket.id,
        userId: String(user.id),
        displayName: String(displayName || user.first_name || 'User').slice(0, 100),
      })
      // Emit call:started when first peer announces
      const roomSockets = io.sockets.adapter.rooms.get(room)
      const count = roomSockets ? roomSockets.size : 1
      if (count >= 1) {
        io.to(room).emit('hangout:call:started', {
          groupId: gid,
          startedBy: { firstName: user.first_name, username: user.username },
        })
      }
    })

    socket.on('webrtc:offer', ({ groupId, to, sdp } = {}) => {
      const gid = parseInt(groupId, 10)
      if (!Number.isFinite(gid)) return
      if (typeof to !== 'string' || typeof sdp !== 'object') return
      // Validate SDP shape (must have type and sdp string fields)
      if (!sdp || typeof sdp.type !== 'string' || typeof sdp.sdp !== 'string') return
      if (sdp.sdp.length > 65536) return // max 64 KB SDP
      const room = `hangout:${gid}`
      if (!socket.rooms.has(room)) return
      io.to(to).emit('webrtc:offer', { from: socket.id, sdp })
    })

    socket.on('webrtc:answer', ({ groupId, to, sdp } = {}) => {
      const gid = parseInt(groupId, 10)
      if (!Number.isFinite(gid)) return
      if (typeof to !== 'string' || typeof sdp !== 'object') return
      if (!sdp || typeof sdp.type !== 'string' || typeof sdp.sdp !== 'string') return
      if (sdp.sdp.length > 65536) return
      const room = `hangout:${gid}`
      if (!socket.rooms.has(room)) return
      io.to(to).emit('webrtc:answer', { from: socket.id, sdp })
    })

    socket.on('webrtc:ice', ({ groupId, to, candidate } = {}) => {
      const gid = parseInt(groupId, 10)
      if (!Number.isFinite(gid)) return
      if (typeof to !== 'string') return
      // candidate can be null (end-of-candidates signal) or an RTCIceCandidateInit object
      if (candidate !== null && typeof candidate !== 'object') return
      if (candidate && typeof candidate.candidate !== 'string') return
      if (candidate && candidate.candidate.length > 4096) return
      const room = `hangout:${gid}`
      if (!socket.rooms.has(room)) return
      io.to(to).emit('webrtc:ice', { from: socket.id, candidate })
    })

    socket.on('webrtc:leave', ({ groupId } = {}) => {
      const gid = parseInt(groupId, 10)
      if (!Number.isFinite(gid)) return
      const room = `hangout:${gid}`
      socket.to(room).emit('webrtc:user-left', { peerId: socket.id, userId: String(user.id) })
      // Emit call:ended when the room has no peers remaining
      const roomSockets = io.sockets.adapter.rooms.get(room)
      const count = roomSockets ? roomSockets.size : 0
      if (count <= 1) {
        io.to(room).emit('hangout:call:ended', { groupId: gid })
      }
    })
