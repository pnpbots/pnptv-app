...
/**
 * broadcast-chasetheclouds-live-2026-07-16.js
 *
 * Massive broadcast announcing chasetheclouds is live on PNP Live.
 * Includes link preview card so Telegram renders the stream page.
 *
 * Usage:
 *   docker exec pnptv-bot node apps/backend/scripts/broadcast-chasetheclouds-live-2026-07-16.js --dry-run
 *   docker exec pnptv-bot node apps/backend/scripts/broadcast-chasetheclouds-live-2026-07-16.js
 *   docker exec pnptv-bot node apps/backend/scripts/broadcast-chasetheclouds-live-2026-07-16.js --force
 */
...
const ENTITY_ID  = 'chasetheclouds-live-2026-07-16';
const LIVE_URL   = 'https://pnptv.app/live/pnptv-chasingthert';
...
const TG = {
  en: (name) =>
`🔴 <b>LIVE NOW — chasetheclouds is streaming on PNPtv!</b>

Hey ${name}, Chase The Clouds just went live on PNP Live. Drop in, say hi, and show some love 🙌

${LIVE_URL}`,

  es: (name) =>
`🔴 <b>EN VIVO AHORA — chasetheclouds está transmitiendo en PNPtv!</b>

Hey ${name}, Chase The Clouds acaba de salir en vivo en PNP Live. Entra, salúdalo y demuestra tu apoyo 🙌
...
async function main() {
  console.log('\n═══════════════════════════════════════════════════');
  console.log(' chasetheclouds LIVE Broadcast — July 16 2026');
  console.log('═══════════════════════════════════════════════════');
  ...
  for (let i = 0; i < targets.length; i++) {
    ...
    try {
      await tg.sendMessage(u.telegram, msg, {
        parse_mode: 'HTML',
        // Link preview enabled — lets Telegram render the stream page card
      });
      ...
      `, [
        ...
        ENTITY_ID,
        lang === 'en'
          ? '🔴 chasetheclouds is LIVE on PNP Live — join the stream!'
          : '🔴 chasetheclouds está EN VIVO en PNP Live — ¡únete al stream!',
        JSON.stringify({ liveUrl: LIVE_URL }),
      ]);
