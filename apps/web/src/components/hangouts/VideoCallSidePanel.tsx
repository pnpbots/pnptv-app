      ...
      navigator.geolocation.getCurrentPosition(
        async (pos) => {
          try {
            ...
            if (!cancelled) {
              // Free tier returns { count }, paid tier returns { total }
...
function SidePanelChat({ groupId, userId }: { groupId: number; userId: string }) {
      ...
      <div ref={scrollRef} className="flex-1 overflow-y-auto p-2 space-y-1.5 scrollbar-thin">
        ...
        {messages.map((msg) => (
          <div key={msg.id} className="flex items-start gap-1.5 group">
            {msg.photo_url ? (
              <img
                src={msg.photo_url.startsWith("http") ? msg.photo_url : msg.photo_url}
                alt=""
