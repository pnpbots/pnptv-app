...
interface VideoCallSidePanelProps {
  ...
  /** Whether the current user is a moderator or call creator */
  isModerator?: boolean;
  /** Whether this panel is inside the Main Stage (Radio PNP always plays) */
  isMainStage?: boolean;
}
...
export function MobileBottomBar({ groupId, userId, isAdmin = false, isLandscape = false, onOpenModBot, socketChat, isModerator = false, isMainStage = false }: MobileBottomBarProps) {
  ...
  const sheetContent = activeTab && activeTab !== "mod" ? (
    <>
      ...
      {activeTab === "radio" && (
        <div className="flex-1 overflow-y-auto p-3 space-y-2">
          <p className="text-[10px] text-pnp-textSecondary/60 uppercase tracking-wide">
            {isMainStage ? "Radio PNP" : "Music"}
          </p>
          <HangoutMusicBar
            groupId={groupId}
...
export function VideoCallSidePanel({
  ...
  return (
    <div className="flex flex-col w-72 max-h-full bg-pnp-background/80 backdrop-blur-sm rounded-xl border border-white/5 overflow-hidden">
      ...
      <div className="px-2 pb-1 space-y-1 flex-shrink-0">
        <p className="text-[9px] text-pnp-textSecondary/60 uppercase tracking-wide px-1 pt-1">
          {isMainStage ? "Radio PNP" : "Music"}
        </p>
        <HangoutMusicBar
          groupId={groupId}
