            ...
            <div className="space-y-4">
              ...
              <div>
                ...
                <input
                  ...
                  onChange={(e) => {
                    set("logo_url", e.target.value || null);
                    setLogoError(false);
                  }}
                  placeholder="https://example.com/logo.png"
                  className="w-full rounded-lg bg-pnp-surface border border-pnp-border px-3 py-2.5 text-sm text-pnp-textPrimary placeholder-pnp-textSecondary focus:outline-none focus:ring-2 focus:ring-pnp-accent transition-colors"
                />
                {/* Inline logo preview thumbnail */}
                {draft.logo_url && (
                  <div className="mt-2 flex items-center gap-3">
                    <div className="w-12 h-12 rounded-lg bg-pnp-surfaceHover flex items-center justify-center overflow-hidden border border-pnp-border">
                      {!logoError ? (
                        <img
                          src={draft.logo_url}
                          alt="Logo preview"
                        ...
                        <span className="text-[10px] text-pnp-error text-center px-1">
                          Load failed
                        </span>
                      )}
                    </div>
                    {logoError && (
                      <p className="text-xs text-pnp-error">
                ...
                {/* Browse logo library */}
                <button
                  onClick={() => setShowLogoLibrary(!showLogoLibrary)}
                  className="w-full mt-2 py-2 rounded-lg border border-dashed border-pnp-border text-xs font-medium text-pnp-textSecondary hover:border-pnp-accent/40 hover:text-pnp-accent transition-colors"
                >
                  {showLogoLibrary ? "Hide Library" : "Browse Logo Library"}
                </button>
                {showLogoLibrary && (
                  <div className="mt-3 p-3 rounded-xl bg-pnp-background border border-pnp-border">
                    <AssetLibraryPicker
                      type="logo"
                      onSelect={(asset) => {
                        set("logo_url", asset.image_url);
                        setLogoError(false);
            ...
            <div className="space-y-4">
              ...
              <div>
                ...
                <input
                  ...
                  onChange={(e) =>
                    set("banner_image_url", e.target.value || null)
                  }
                  placeholder="https://cms.pnptv.app/assets/..."
                  className="w-full rounded-lg bg-pnp-surface border border-pnp-border px-3 py-2.5 text-sm text-pnp-textPrimary placeholder-pnp-textSecondary focus:outline-none focus:ring-2 focus:ring-pnp-accent transition-colors"
                />
                {draft.banner_image_url && (
                  <div className="mt-2 flex items-center gap-3">
                    <div className="h-10 rounded-lg bg-pnp-surfaceHover flex items-center justify-center overflow-hidden border border-pnp-border px-2">
                      <img
                    ...
                    <button
                      ...
                      className="text-[10px] text-pnp-error hover:underline"
                    >
                      Remove
                    </button>
                  </div>
                ...
                <p className="text-[10px] text-pnp-textSecondary/60 mt-1">
                  Image banner takes priority over text banner when both are
                  set.
                </p>
                {/* Browse banner library */}
                <button
                ...
                {showBannerLibrary && (
                  <div className="mt-3 p-3 rounded-xl bg-pnp-background border border-pnp-border">
                    <AssetLibraryPicker
                      type="banner"
                      onSelect={(asset) => {
                        set("banner_image_url", asset.image_url);
