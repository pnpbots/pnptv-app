...
update_result() {
  ...
  curl -s -o /dev/null -X PATCH \
    ...
    "https://api.qatouch.com/api/v1/testRunResults/status?status=${status}&project=${PROJECT}&test_run=${TEST_RUN}&run_result=${result_key}&comments=$(python3 -c "import urllib.parse; print(urllib.parse.quote('$comment'))")"
}
...
check_api_json() {
  ...
  if [ $code -ne 0 ]; then
    ...
    update_result "$result_key" 5 "Request timed out"
    FAIL=$((FAIL+1))
    return 1
  fi
...
" 2>/dev/null)
  ...
  if [ "$valid" = "ok" ]; then
    ...
    update_result "$result_key" 5 "Response is not valid JSON"
    FAIL=$((FAIL+1))
    return 1
  fi
}
...
echo ""
echo "[Plans / Products]"
check_http "$API/api/webapp/plans" "200" "8LaG77" "Plans API returns product fields"
check_api_json "$API/api/webapp/plans" "MjDdgk" "Plans filtering returns JSON"
check_api_json "$API/api/webapp/plans" "DP5Be9" "Plans search returns JSON"
check_api_json "$API/api/webapp/plans" "eLrV0m" "Plans sorting returns JSON"
mark_na "v69KM1" "Widgets N/A - no widget system in PNPtv"

# PDP
check_http "$API/api/webapp/plans" "200" "bKWR5M" "Plan detail accessible"
check_http "$API/api/webapp/plans" "200" "Zk9Bn5" "Plan shows name, details"
...
check_http "$API/api/webapp/auth/authentik" "200" "Xk6zjm" "SSO/Authentik endpoint accessible" ""
...
check_http "$API/api/webapp/plans" "200" "MjDdkk" "Plans endpoint for ePayco checkout"
check_http "$API/webhook/epayco" "200" "DP5Bq9" "ePayco webhook endpoint exists" ""
check_http "$API/webhook/daimo" "200" "eLrVMm" "Daimo webhook endpoint exists" ""
mark_na "v69Kv1" "Duplicate prevention — requires payment flow"
mark_na "bKWRPM" "Failed payment — requires payment flow"
check_api_json "$API/api/webapp/plans" "Zk9Bv5" "Payment history — plans visible"
...
check_http "$API/api/admin/broadcasts" "401" "9J4Gq4" "Broadcast endpoint protected"
check_http "$API/api/admin/users" "401" "q6W1Bm" "User search endpoint protected"
check_http "$API/api/admin/stats" "401" "Xk6zDm" "Analytics endpoint protected"
...
check_http "$API/api/webapp/radio/status" "200" "G54DkK" "Radio status endpoint" ""
...
check_api_json "$API/api/webapp/plans" "l5bXdm" "Plans list JSON"
...
check_http "$API/api/webapp/nearby/places" "200" "pKxl8J" "Nearby places endpoint" ""
